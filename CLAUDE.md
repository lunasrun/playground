...
2. **Playground UI in Lunas** — reproduce the previous Material-ish look (this
   repo's prior Vue/Vuetify implementation, in git history) with VS Code-like
   multi-file management and live in-browser compile.
3. **Editor + language server** — Monaco with syntax highlighting, wired to the
   Lunas language server (from the `lunasrun/tools` repo) over a worker.
